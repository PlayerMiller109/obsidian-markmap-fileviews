const newRgx = (query, flags)=> new RegExp(String.raw`${query}`, flags)
const list = '\n?^ *?(-|\d+?\.) ', heading = '#{1,6} '
const headingInList = `(?<=${list})${heading}`
const keepText = new class {
  listRgx = newRgx(list, 'gm')
  headRgx = newRgx(`${headingInList}|^${heading}`, 'g')
...
module.exports = (app, ob)=> {
  ...
  return md2htmlText = async (mdText, sourcePath)=> {
    const tohtml = async (seg)=> {
      ...
      inlink2btn(el, sourcePath); const cEl = el.children[0]
      return cEl?.tagName == 'P' ? cEl.innerHTML : el.innerHTML
    }
    const r = []
    for (const sec of mdText.split('\n\n')) {
      ...
      _r = _r.replace(newRgx(headingInList, ''), '')
      if (_r) r.push(_r)
    }
    return r.join('\n\n')
  }
