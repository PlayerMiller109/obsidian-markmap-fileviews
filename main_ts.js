...
const import_poper = (app, ob)=> {
  const userLeaf = (ctx, parent, linkpath, rPath)=> new class extends ob.WorkspaceLeaf {
    ...
    load = async ()=> {
      ...
      Object.assign(this.view.contentEl, {
        onmousedown: this.onDrag,
        onkeydown: evt=> { evt.ctrlKey && evt.altKey && this.active() },
      })
    }
    active = ()=> setTimeout(()=> (app.workspace.activeLeaf = this, this.view.editor.focus()), 280)
    onDrag = (evt)=> {
      this.active(); this.togglePin(!0); if (!evt.ctrlKey) return
...
const import_patch = (app, ob)=> {
  ...
  const userHover = (view, target, linkpath, rPath)=> {
...
const import_mmPlug = (app, ob)=> {
  ...
  return function() {
    if (mmJson.handlePagePreview) this.app.workspace.onLayoutReady(()=> patcher.call(this))
